...
void squares(int size1, int x_offset, int y_offset, int size2) {
  ...
  for (int y = 0; y < h; y++){
    ...
    for (int x = 0; x < w; x++){
      ...
      // if so, print a *
      if (isAtBorder(y, y_offset, size2 - 1) && isInRange(x, x_offset, size2)){
	printf("*");
      } else if (isAtBorder(x, x_offset, size2 - 1) && isInRange(y, y_offset, size2)){
	printf("*");
      }
      //if not,
      // check if EITHER
      ...
      //if so, print a #
        else if (isAtBorder(y, 0, size1 - 1) && isInRange(x, 0, size1)){
	printf("#");
      } else if (isAtBorder(x, 0, size1 - 1) && isInRange(y, 0, size1)){
	printf("#");
    }
      //else print a space
        else {
